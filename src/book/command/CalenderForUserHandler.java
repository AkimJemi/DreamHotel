...
public class CalenderForUserHandler implements CommandHandler {
	...
	private int room_no, room_num, start_date, end_date;

	@Override
	public String process(HttpServletRequest rq, HttpServletResponse rp) throws Exception {
		System.out.println("firstLine start_date : " +start_date);
		System.out.println("firstLine end_date : " +end_date);
		if (rq.getParameter("check") != null) {
			...
			start_date = Integer.parseInt(rq.getParameter("start_date").substring(8, 10));
			end_date = Integer.parseInt(rq.getParameter("end_date").substring(8, 10)) -1;
			int start_dates = start_date;
			int end_dates = end_date;
			for (int j = 1; j <= end_dates - start_dates; j++) {
				calDates.add(start_dates);
				start_dates++;
				j--;
			}
			System.out.println("init start_date : " +start_date);
			System.out.println("init end_date : " +end_date);
		...
		System.out.println("after start_date : " +start_date);
		System.out.println("after end_date : " +end_date);
...
//				((String) map.get("start")).substring(0, 10), ((String) map.get("end")).substring(0, 10));
		...
		System.out.println("test : " + start_date + end_date);
		rq.setAttribute("json", json);
